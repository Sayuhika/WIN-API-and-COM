...
int main()
{
	HINSTANCE library = ::LoadLibrary(L"COMDLL.dll");
	if(library == NULL)
	{
		return 1;
	}

	CREATEINSTANCEPROC CreateInstance = (CREATEINSTANCEPROC)::GetProcAddress(library, "CreateInstance");
	...
	ICollection* pICollection = 0;
	hr = pIUnknown->QueryInterface(IID_ICollection, (void**)&pICollection);
	...
	IEnumerator* pIEnumerator = 0;
	hr = pIUnknown->QueryInterface(IID_IEnumerator, (void**)&pIEnumerator);
	if (SUCCEEDED(hr))
	{
